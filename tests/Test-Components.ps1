...
if ($providerCliLoaded) {
    ...
    $pinnedId = $null
    try {
        $pinnedId = Resolve-ProviderModelId -ModelAlias "Opus-4-7" -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Resolve-ProviderModelId maps Opus-4-7 to pinned id" `
        -Condition ($pinnedId -eq "claude-opus-4-7") `
        -Message "Expected claude-opus-4-7, got $pinnedId"

    $opusVersion = $null
    try {
        $opusVersion = Resolve-ProviderModelVersion -ModelAlias "Opus" -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Resolve-ProviderModelVersion maps Opus to claude-opus-4-7" `
        -Condition ($opusVersion -eq "claude-opus-4-7") `
        -Message "Expected claude-opus-4-7, got $opusVersion"

    $passthroughVersion = $null
    try {
        $passthroughVersion = Resolve-ProviderModelVersion -ModelAlias "claude-opus-4-7" -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Resolve-ProviderModelVersion accepts pinned id" `
        -Condition ($passthroughVersion -eq "claude-opus-4-7") `
        -Message "Expected claude-opus-4-7, got $passthroughVersion"

    $versionUnknownError = $false
    try {
        Resolve-ProviderModelVersion -ModelAlias "Frobnicator" -ProviderName "claude"
    }
    catch {
        $versionUnknownError = $true
    }

    Assert-True -Name "Resolve-ProviderModelVersion throws on unknown alias" `
        -Condition $versionUnknownError `
        -Message "Should throw for unknown model alias"

    $opusPricing = $null
    try {
        $opusPricing = Get-ProviderModelPricing -ModelAlias "Opus" -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Get-ProviderModelPricing returns Opus input rate" `
        -Condition ($null -ne $opusPricing -and $opusPricing.InputPerMTok -eq 5.00) `
        -Message "Expected InputPerMTok=5.00, got $($opusPricing.InputPerMTok)"
    Assert-True -Name "Get-ProviderModelPricing returns Opus output rate" `
        -Condition ($null -ne $opusPricing -and $opusPricing.OutputPerMTok -eq 25.00) `
        -Message "Expected OutputPerMTok=25.00, got $($opusPricing.OutputPerMTok)"

    $opusModelEntry = $null
    try {
        $opusModelEntry = Get-ProviderModels -ProviderName "claude" |
            Where-Object { $_.Alias -eq "Opus" } |
    ...
    Assert-True -Name "Get-ProviderModels surfaces Version field" `
        -Condition ($null -ne $opusModelEntry -and $opusModelEntry.Version -eq "claude-opus-4-7") `
        -Message "Expected Version=claude-opus-4-7"
    Assert-True -Name "Get-ProviderModels surfaces IsLatest field" `
        -Condition ($null -ne $opusModelEntry -and $opusModelEntry.IsLatest -eq $true) `
        -Message "Expected IsLatest=true for Opus"
    Assert-True -Name "Get-ProviderModels surfaces InputPerMTok field" `
        -Condition ($null -ne $opusModelEntry -and $opusModelEntry.InputPerMTok -eq 5.00) `
        -Message "Expected InputPerMTok=5.00"

    $emptyInputError = $false
    try {
        Resolve-ProviderModelVersion -ModelAlias "" -ProviderName "claude"
    }
    catch {
        $emptyInputError = $true
    }

    Assert-True -Name "Resolve-ProviderModelVersion throws on empty string" `
        -Condition $emptyInputError `
        -Message "Should throw for empty alias"

    $whitespaceInputError = $false
    try {
        Resolve-ProviderModelVersion -ModelAlias "   " -ProviderName "claude"
    }
    catch {
        $whitespaceInputError = $true
    }

    Assert-True -Name "Resolve-ProviderModelVersion throws on whitespace" `
        -Condition $whitespaceInputError `
        -Message "Should throw for whitespace alias"

    $lowercaseVersion = $null
    try {
        $lowercaseVersion = Resolve-ProviderModelVersion -ModelAlias "opus" -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Resolve-ProviderModelVersion is case-insensitive on alias key" `
        -Condition ($lowercaseVersion -eq "claude-opus-4-7") `
        -Message "Expected claude-opus-4-7 for lowercase 'opus', got $lowercaseVersion"

    $pinnedPricing = $null
    try {
        $pinnedPricing = Get-ProviderModelPricing -ModelAlias "Opus-4-7" -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Get-ProviderModelPricing returns Opus-4-7 input rate" `
        -Condition ($null -ne $pinnedPricing -and $pinnedPricing.InputPerMTok -eq 5.00) `
        -Message "Expected InputPerMTok=5.00 for Opus-4-7"
    Assert-True -Name "Get-ProviderModelPricing returns Opus-4-7 output rate" `
        -Condition ($null -ne $pinnedPricing -and $pinnedPricing.OutputPerMTok -eq 25.00) `
        -Message "Expected OutputPerMTok=25.00 for Opus-4-7"

    $unknownPricingError = $false
    try {
        Get-ProviderModelPricing -ModelAlias "NoSuchModel" -ProviderName "claude"
    }
    catch {
        $unknownPricingError = $true
    }

    Assert-True -Name "Get-ProviderModelPricing throws on unknown alias" `
        -Condition $unknownPricingError `
        -Message "Should throw for unknown model alias"

    $crossProviderPricingError = $false
    try {
        Get-ProviderModelPricing -ModelAlias "Opus" -ProviderName "codex"
    }
    catch {
        $crossProviderPricingError = $true
    }

    Assert-True -Name "Get-ProviderModelPricing rejects Opus for codex" `
        -Condition $crossProviderPricingError `
        -Message "Should throw when alias is foreign to provider"

    $codexModels = $null
    try {
        $codexModels = Get-ProviderModels -ProviderName "codex"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }

    Assert-True -Name "Get-ProviderModels returns codex models" `
        -Condition ($null -ne $codexModels -and $codexModels.Count -ge 1) `
        -Message "Expected at least 1 codex model"

    $codexFirst = $null
    if ($codexModels) {
        $codexFirst = $codexModels | Select-Object -First 1
    }

    Assert-True -Name "Get-ProviderModels degrades gracefully when codex omits new fields" `
        -Condition ($null -ne $codexFirst -and $null -eq $codexFirst.Version -and $null -eq $codexFirst.InputPerMTok) `
        -Message "Expected null Version/InputPerMTok for codex (no metadata defined)"

    $claudeModels = $null
    try {
        $claudeModels = Get-ProviderModels -ProviderName "claude"
    }
    catch {
        Write-Verbose "Non-critical operation failed: $_"
    }
    ...
    $claudeAliases = @()
    if ($claudeModels) {
        $claudeAliases = $claudeModels | ForEach-Object { $_.Alias }
    }

    foreach ($canonical in @("Opus", "Sonnet", "Haiku")) {
        Assert-True -Name "claude.json defines canonical alias '$canonical'" `
            -Condition ($claudeAliases -contains $canonical) `
            -Message "Missing canonical alias $canonical in claude.json"
    }
    ...
    if ($claudeModels) {
        foreach ($m in $claudeModels) {
            if ($m.IsLatest -ne $true) {
                continue
            }
            if ($m.Alias -like 'Opus*') {
                $opusLatestCount++
            }
            elseif ($m.Alias -like 'Sonnet*') {
                $sonnetLatestCount++
            }
            elseif ($m.Alias -like 'Haiku*') {
                $haikuLatestCount++
            }
        }
    }

    Assert-True -Name "Exactly one Opus-tier entry has IsLatest=true" `
        -Condition ($opusLatestCount -eq 1) `
        -Message "Expected 1 Opus-tier with IsLatest=true, got $opusLatestCount"
    Assert-True -Name "Exactly one Sonnet-tier entry has IsLatest=true" `
        -Condition ($sonnetLatestCount -eq 1) `
        -Message "Expected 1 Sonnet-tier with IsLatest=true, got $sonnetLatestCount"
    Assert-True -Name "Exactly one Haiku-tier entry has IsLatest=true" `
        -Condition ($haikuLatestCount -eq 1) `
        -Message "Expected 1 Haiku-tier with IsLatest=true, got $haikuLatestCount"

    $sonnetFallback = $null
    if ($claudeModels) {
        $sonnetFallback = $claudeModels |
            Where-Object { $_.Alias -eq 'Sonnet' } |
            Select-Object -First 1
    }

    Assert-True -Name "expand-task-groups Sonnet fallback resolves to non-empty Id" `
        -Condition ($null -ne $sonnetFallback -and -not [string]::IsNullOrWhiteSpace($sonnetFallback.Id)) `
        -Message "Sonnet fallback path returned no usable Id"
