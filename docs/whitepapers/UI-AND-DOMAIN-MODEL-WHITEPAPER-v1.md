# Dotbot UI Architecture & Domain Model Whitepaper v1
...
Dotbot has three UI surfaces emerging in parallel — the **Project Dashboard** (Outpost Control Panel), the **Workflow Editor** (PR #113), and the **Mothership Server** — with no unified architecture governing how they relate, navigate between each other, or share a visual identity. Simultaneously, Erol has proposed enriching the domain model with persons, roles, accountability, escalation paths, and transforming workflow configuration from a development exercise into a visual, no-code activity. The open-source project Paperclip (paperclip.ing) demonstrates a mature organizational metaphor for AI agent orchestration that validates several of these ideas and suggests additional ones worth adopting.

This whitepaper consolidates the vision into a single reference document. Once reviewed, it becomes the basis for GitHub issues on Project #2 (v4 Roadmap).
...
**Outpost Control Panel** — The primary daily interface. Per-project, launched via `.bot/go.ps1`. PowerShell HTTP server + vanilla JS frontend. Seven tabs: Overview, Product, Roadmap, Processes, Decisions, Workflows, Settings. Full CRT retro-futuristic design with configurable theme presets. This is the cockpit where a developer monitors and steers their project's AI processes.

**Mothership Dashboard** — The fleet command center. ASP.NET Core Razor Pages app with vanilla JS. Currently serves Q&A notifications (Overview, By Person, By Project tabs). Will expand to fleet management: instance registry, cross-org decision routing, drone management, team directory, cost/velocity dashboards. Hosted centrally (Azure, on-prem, or self-hosted).

**Workflow Editor** — A global power tool for authoring workflow definitions. React + React Flow + Vite, served by a PowerShell HTTP server. Visual DAG editor for `workflow.yaml` files with drag-and-drop task creation, type-specific property panels, and an integrated recipe file editor. Launched from the Outpost's Workflows tab or via `dotbot editor` CLI. Operates on `~/dotbot/workflows/` (global) or project-scoped workflows.

**Drone** — A headless autonomous worker with no UI. Telemetry and control flow through the Mothership. Reuses the same Runtime and MCP Server as an Outpost but replaces the Dashboard with a lightweight agent supervisor that polls the Mothership work queue.

### 2.3 User Mental Model

> "I open my **project** to work on tasks, check the **mothership** for fleet status, and launch the **editor** when I need to modify workflows."
...
| Process monitoring (start, stop, whisper) | ✦ primary | summary view | — | reports telemetry |
| Product docs (PRD, specs, discovery) | ✦ primary | — | — | — |
| Roadmap / pipeline view | ✦ primary | aggregate view | — | — |
| Decision records | ✦ create & manage | ✦ route & approve | — | — |
...
| Drone management / work queue | — | ✦ primary | — | agent |
| Cost / velocity dashboards | per-project stats | ✦ fleet-wide | — | — |
| Extension registries | consumes | ✦ discovery hub | — | consumes |
...
Role {
  ...
  approval_authority: boolean // NEW — can this role approve outputs?
}
```
...
#### 6.2.3 Governance & Audit (inspired by Paperclip)

Paperclip treats AI agents as employees in an org chart with full audit trails, budget enforcement, and board-level governance. Dotbot's equivalents:
...
Dotbot doesn't track costs today. Phase 8's gateway routing mode and observability suite lay the groundwork. Cost policies could be defined per-project in settings and enforced by the Runtime's provider abstraction.
...
This aligns with Erol's nudging strategy — the system can autonomously nudge humans on schedule, not just react to events.

#### 6.2.4 Organizational Hierarchy (Paperclip-inspired, evaluate carefully)

Paperclip models an entire company with reporting structures and delegation chains. Dotbot should **not** adopt this fully — dotbot is project-centric, not company-centric. However, the delegation concept maps well:
...
This is scoped to the project team, not an org chart. The Mothership aggregates team visibility across projects for org-wide awareness, but authority stays local.
...
Erol's core insight: **creating new dotbot configurations should be a configuration exercise, not a development task.** The workflow editor (PR #113) is the first step. The long-term vision is a visual tool that lets users:
...
3. **Browse shared libraries** — when connected to a Mothership, discover and import workflows, skills, and agents from extension registries (Phase 11)
4. **Create new process types** — kickstart-via-jira, idea intake, product proposal — without writing PowerShell
5. **Configure pipeline phases** — visual definition of kickstart pipelines with LLM, interview, workflow, and script phases

### 7.2 PR #113: Current State
...
**Phase 2 (Post Phase 6 — Stacks vs Workflows):** Once profiles are split into stacks and workflows, the editor manages both independently. Stacks define technology layers; workflows define process pipelines. The editor gets a mode switch.

**Phase 3 (Post Phase 7 — Workflow Isolated Runs):** The editor gains a "Run" button that creates a workflow run directly from the editor canvas. Live run status overlays on nodes (green=done, amber=running, red=failed).

**Phase 4 (Post Phase 11 — Extension Registries):** The editor connects to registries. A "Library" panel shows available workflows, skills, and agents from configured registries. Import with one click. Publish back to a registry. When connected to a Mothership, the library spans all registered registries.

**Phase 5 (Kickstart Pipeline Designer):** The editor supports kickstart pipeline definition — the multi-phase onboarding flow (LLM analysis → interview → workflow generation → script execution). Pipeline phases are visual blocks with configurable inputs, outputs, and branching logic.

### 7.4 Standalone vs Mothership-Connected
...
| **Budget Enforcement** | Per-agent monthly USD limits, hard stops at 100% | Not implemented — Phase 8 gateway routing lays groundwork |
| **Heartbeat Scheduling** | Agents wake on schedules or triggers | Steering heartbeat exists; scheduled launches planned |
| **Delegation** | Work flows bidirectionally through org chart | Erol's escalation paths align; dotbot keeps it project-scoped |
| **Ticket Threading** | Persistent conversations with full context | Tasks have activity logs; decisions have discussion threads |
| **Goal Alignment** | Tasks trace to missions | Product docs (PRD, mission, roadmap) provide context but aren't formally linked to tasks |
| **Audit Trails** | Immutable logs of all decisions and tool calls | activity.jsonl exists but lacks governance structure |
...
1. **Cost awareness** — Dotbot should track and display per-task and per-project LLM costs. Not necessarily hard budget enforcement initially, but visibility. Phase 8's observability suite is the right place.

2. **Governance-grade audit** — Extend activity.jsonl with correlation IDs (Phase 1 plans this), actor attribution, and action classification. Make it queryable via MCP tools.

3. **Scheduled agent wakeups** — Support cron-style process scheduling, not just on-demand launches. Useful for nightly analysis sweeps, periodic health checks, and Erol's nudging strategies.
...
5. **MCP tool ecosystem** — 26 auto-discovered tools with metadata-driven registration. Paperclip uses skills injection via markdown files.
...
| **Drone Management** | Active drones, work queue, assignment status | Phase 10 |
| **Decision Hub** | Cross-project decision routing and approval | Phase 5 + 8 |
| **Team Directory** | Org-wide team members aggregated from all outposts | Phase 14 |
| **Cost Dashboard** | Fleet-wide LLM spend, per-project breakdown, trend analysis | Phase 8 |
| **Velocity Metrics** | Tasks completed, cycle time, quality trends | Phase 8 |
| **Extension Library** | Registry browser for shared workflows, skills, agents | Phase 11 |
...
### Phase C: Workflow Editor CRT Integration (builds on PR #113)
**Goal:** Bring the editor into the CRT family.
...
5. Add "Visual Editor" button to Outpost Workflows tab
6. Add "← Back to Control Panel" link in editor header
7. Wire file watcher so Outpost detects editor saves
...
**Goal:** Implement team, accountability, and governance entities.
...
5. Implement nudging strategy (scheduled nudge events via event bus)
6. Add delegation/escalation logic to Q&A routing
7. Add cost tracking to process/task records
8. Extend activity.jsonl with governance audit fields
9. Add team sync endpoint to Mothership
10. Add Team Directory page to Mothership dashboard

### Phase E: Visual Config Tool Evolution
**Goal:** Expand the editor beyond workflow YAML.
1. Add "Run Workflow" button that creates a live workflow run from the canvas
2. Add registry library panel for browsing/importing shared components
...
1. **Cost tracking granularity** — Should costs be tracked per-task, per-process, or per-LLM-call? Per-call gives the most flexibility but requires provider-level metering.

2. **Nudging implementation** — Should nudging be an event bus concern (scheduled events) or a dedicated NudgeService? The event bus approach is more composable but may be overkill for v1.

3. **Workflow editor scope** — Should the editor also manage MCP tool definitions (metadata.yaml + script.ps1)? Or is that too close to development territory for a visual config tool?

4. **Approval gates UX** — Where do humans approve outputs? In the Outpost Decisions tab? Via mothership notification channels (Teams, Slack, email)? Both?

5. **Drone UI** — Drones are headless, but should the Mothership provide a "drone console" view that shows what a drone is doing in real-time (streamed process output)?

---

## 12. Summary

Dotbot's three UI surfaces serve distinct purposes and should remain independent apps linked by contextual navigation. The CRT design system unifies them visually through shared CSS tokens and effects. The domain model needs enrichment in three directions: team/accountability (persons, roles, escalation), governance (audit trails, approval gates, cost controls), and workflow configurability (visual editor evolution to a no-code platform). Paperclip validates the organizational and governance concepts but dotbot's project-centric, file-based, two-phase architecture remains differentiated.

**Next steps:** Review this document → create GitHub issues on Project #2 → prioritize against existing v4 phases.
