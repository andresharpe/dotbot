...
Introduce a per-user runtime process discoverable via `~/.dotbot/`. It exposes a small HTTP API for task and workflow operations, authenticates with a bearer token, and is the sole writer of project state. Projects register themselves with the runtime on `dotbot init`. The UI and MCP layers become clients of this runtime. Endpoint discovery falls back through env vars → settings → `~/.dotbot/runtime.json` so callers always know where to look.

## User Stories

1. As a developer, I want one runtime process per user per machine, so that all my projects share a single source of truth for state mutations.
2. As a developer, I want the runtime's address and credentials in `~/.dotbot/runtime.json` with restricted file permissions, so that other users on a shared machine cannot read my token.
...
5. As a developer, I want `dotbot init` to register my project with the runtime, so that the runtime can later resolve a project path or ID without ambiguity.
6. As a developer with multiple projects, I want the runtime to look up the project on every API call, so that I can't accidentally mutate the wrong project's state.
7. As a developer, I want the runtime to resolve its endpoint via env vars first, then settings, then `~/.dotbot/runtime.json`, so that automated environments (CI, tests) can override the runtime location.
...
12. As a developer running diagnostic checks, I want `dotbot runtime-status` to show PID, URL, registered projects, and active runs, so that I can verify the runtime is healthy and see what it's tracking.
...
15. As a developer using the MCP server in a Claude session, I want the MCP server to discover the runtime token automatically from `~/.dotbot/`, so that I don't have to configure each tool individually.
...
Two files live under `~/.dotbot/`:
- `runtime.json` — `{ url, token, pid, started_at }`. Restricted permissions: mode 600 on POSIX; user-only ACL on Windows.
- `projects.json` — list of `{ id, path, registered_at }`. Project IDs are `p_` + 8-char nanoid.

The runtime is a single PowerShell process per user. It owns:
...
- A `project_id` (or `project_path`) parameter on every API call, validated against the registry (404 if unknown).
...
- Endpoint discovery exported as `Resolve-RuntimeEndpoint`, used by both MCP tools (PRD-07) and the UI proxy (PRD-08): env vars `DOTBOT_RUNTIME_URL`/`DOTBOT_RUNTIME_TOKEN` > merged settings > `~/.dotbot/runtime.json`.

The HTTP surface (used by PRD-07 MCP and PRD-08 UI):

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/projects` | Register project (idempotent for same path) |
...
Lifecycle:
- `dotbot go` checks `~/.dotbot/runtime.json`. If file exists and its PID is alive, the runtime is already up; attach. Otherwise: generate fresh 64-hex-char token, pick a port, start the listener, write `runtime.json` with restricted permissions.
...
The project registry, runtime-client helpers, mutex pool, and endpoint discovery all live inside the `Dotbot.Runtime` module — no separate `Dotbot.ProjectRegistry` or `Dotbot.RuntimeClient` modules. The MCP and UI clients import `Resolve-RuntimeEndpoint` and a thin `Invoke-RuntimeRequest` helper from `Dotbot.Runtime`.

`dotbot init` calls the runtime's `POST /projects` after the standard init steps. If the runtime isn't running it prints a clear message asking the user to start it.
...
- **Dotbot.Runtime** (HTTP API) — every endpoint with valid auth = 2xx with expected body shape; missing/wrong auth = 401; unknown project = 404; illegal transition = 422; non-isolated concurrent run = 409.
...
- **Dotbot.Runtime** (endpoint discovery) — set env var → returns env; unset env, set settings → returns settings; unset both → returns `~/.dotbot/runtime.json` content; nothing available → throws "runtime not running".
- **Dotbot.Runtime** (project registry) — register adds; register same path is idempotent; lookup by path returns id; lookup by unknown id raises a missing-project response.
- **Dotbot.Runtime** (lifecycle) — stale-PID runtime.json is rewritten with a fresh token on `dotbot go`; shutdown cleans up.
