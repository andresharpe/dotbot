...
> **Keep `.bot/workspace/` tracked in git; let `dotbot init` gitignore the rest.**
>
> - Framework files under `.bot/core/`, `.bot/hooks/`, `.bot/recipes/`, `.bot/settings/`, plus top-level `*.ps1`, `README.md`, and `.manifest.json`, are **installed locally by `dotbot init` and gitignored** in the consumer repo. Each developer's machine gets its own copy; the framework is not committed. `dotbot init` adds the necessary entries to your project's `.gitignore` automatically and untracks any previously-committed framework files in a one-time migration commit.
> - Workspace content under `.bot/workspace/` (tasks, decisions, plans, product docs) **stays tracked** so teams can share it. If you add `.bot/workspace/` to `.gitignore`, `dotbot init` warns and asks you to remove the rule.
> - Runtime state (`.bot/.control/`, `.bot/profile/`, `.bot/.chrome-dev/`, `.bot/workspace/sessions/runs/`) is gitignored too. These hold per-developer process state and should never enter version control.
> - Framework files are protected by a SHA256 manifest at `.bot/.manifest.json` (per-developer, gitignored, regenerated on every `dotbot init`). The verify hook hashes the locally-installed framework files and compares against the manifest to catch tampering. Direct edits are detected on the next task gate; run `dotbot init --force` to regenerate after intentional changes.
> - Worktree state sharing still works — it uses Windows junctions / Unix symlinks at the filesystem level, independent of git tracking.
