<#
.SYNOPSIS
Claude stream parser for Dotbot.Provider.

.DESCRIPTION
Processes Claude CLI stream-json output lines. Used as a fallback; the primary
Claude path delegates to Invoke-ClaudeStream in Dotbot.Provider directly. Provides
Process-StreamLine for the provider stream dispatcher.

Dot-sourced from Invoke-ProviderStream in Dotbot.Provider.psm1, so Write-ClaudeLog
and Write-ActivityLog are already in scope via the parent module.
#>
