...
<#
.SYNOPSIS
Unified AI provider integration: Claude streaming, provider-agnostic dispatch,
failure classification, rate-limit parsing, provider-session cleanup.

.DESCRIPTION
Single module covering everything dotbot needs to talk to an AI provider:

  - Provider config loading from settings/providers/{name}.json
  - CLI argument building per provider capabilities
  - Streaming invocation (Invoke-ProviderStream) with per-provider stream parsers
  - Claude-specific streaming engine (Invoke-ClaudeStream) for the proven path
  - Activity log writing for the oscilloscope UI
  - Rate-limit message parsing (Get-RateLimitResetTime)
  - Failure classification (Get-FailureReason)
  - Provider-session cleanup (Remove-ProviderSession)
...
# Console theming for streaming output
...
# PathSanitizer (from MCP layer) strips absolute paths from activity log messages.
# $PSScriptRoot is src/runtime/Modules/Dotbot.Provider/; three levels up reaches src/.
...
#region Stream parsing helpers (Claude-format)
...
#region Provider dispatch (Claude / Codex / Gemini)

if (-not (Get-Module Dotbot.Core)) {
    Import-Module (Join-Path $PSScriptRoot '..' 'Dotbot.Core' 'Dotbot.Core.psm1') -DisableNameChecking
}

# Dotbot.Theme is already imported at the top of this file for the
# Claude streaming engine; ditto Dotbot.Settings is loaded here for
# provider selection. Claude helpers (Invoke-ClaudeStream, Write-ActivityLog,
# etc.) live in this same module above so no separate import is needed.
...
#region Provider session cleanup
# Removes local artifacts from a provider session. Claude stores per-project
# session folders under ~/.claude/projects/<hash>/; Codex/Gemini have no local
# artifacts to clean.
...
function Remove-ProviderSession {
    ...
    # Determine active provider — Get-ProviderConfig is defined in this same module.
    $providerName = 'claude'
    try {
...
#region Aliases (Claude back-compat)
